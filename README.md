...
- B agent 每 30 秒读取一次 `wg show wg0 latest-handshakes`。
- 如果 A 这个 peer 的最近握手超过 180 秒，记为一次失败。
- 连续 3 次失败后，B 上报 `peer.unreachable`。
- VPS daemon 收到后自动给 `home-a/wg0` 下发 `natter.run`。
- 同一个 server/interface 默认 5 分钟冷却，避免频繁重启 A 的 WireGuard；可以通过 daemon 的 `--natter-cooldown` 调整。

监控目标不需要在 B 的配置里重复填写。B agent 每次 poll daemon 时，daemon 会根据 binding 自动告诉 B 应该监控哪些 peer。
...
VPS daemon 不提供 HTTP 服务。需要可视化时，在你自己的电脑或内网管理机上启动本地 Web UI：

```sh
./wgnh web \
  --addr 127.0.0.1:9090 \
  --daemon-addr 你的VPS域名:3333 \
  --admin-token "$ADMIN_TOKEN"
```
...
这个 Web UI 只作为本地管理界面，浏览器访问的是本机 `127.0.0.1:9090`；它再通过自定义 TCP 协议连接 VPS daemon。
...
- 查看最近事件。
- 点击按钮手动触发某个 server/interface 的 natter。
...
- 还没有 Web UI。
- 自动检测目前基于客户端 `wg show latest-handshakes`，还没有更复杂的多客户端聚合策略。
